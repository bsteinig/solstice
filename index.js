...
app.get("/", (req, res) => {
  res.status(200).send({ text: "hi paul", text2: "hi zay" });
});

app.get("/dates/:year", (req, res) => {
  const { year } = req.params;
  res.status(200).send(calc(year));
});

app.listen(port, () => {
  console.log(`Solstice is listening on port http://localhost:${port}`);
});
...
function calc(year) {
  ...
  let sol = { data: [] };
  for (var i = 1; i <= 4; i++) {
    // Loop over 4 events: 1=AE, 2=SS, 3-VE, 4=WS
    data = calcEquiSol(i, year); // This routine calculates and displays a single event
    let date = data[0],
      years = data[1];
    sol.data.push({ date: date, year: years });
  }
  return sol;
} // End calc
...
function calcEquiSol(i, year) {
  ...
  return [i, str]; // Output date & time of event in Local Time
} // End calcEquiSol

//-----Calcualte an initial guess as the JD of the Equinox or Solstice of a Given Year
// Meeus Astronmical Algorithms Chapter 27
function calcInitial(k, year) {
...
function fromTDTtoUTC(tobj) {
  ...
  if (Year >= TBLfirst && Year <= TBLlast) {
    ...
    if (Year % 2) {
      ...
      deltaT =
        (TBL[(Year - TBLfirst - 1) / 2] + TBL[(Year - TBLfirst + 1) / 2]) / 2;
    } else {
